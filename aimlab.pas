//Aim Lab 0.1 by mipHo
//Based on Run Mode 2.5 by Savage
...
var
	...
	_LastDamage, _Damage: array[1..32] of Single;
	_isPlaying: array[1..32] of Boolean;
	_Timer: array[1..32] of TDateTime;
	_RealTimer: array[1..32] of TDateTime;
	_AssociatedBot: array[1..32] of Integer;
	_ShotsFired, _ShotsHit, _HeadShots: array[1..32] of Integer;
	{$IFDEF RUN_MODE_DEBUG}
		_PingRespawn: array[1..32] of Integer;
	{$ENDIF}
	_JustFinished, _ShowDamage, _ShowTimer, _RKill, _LoggedIn, _JustDied, _FlightMode, _GodMode: array[1..32] of Boolean;
...
{$IFDEF AIMLAB}
	...
	begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Map, Score FROM Scores ORDER BY Map, Score DESC, HeadShots DESC;') then
			...
			While DB_NextRow(DB_ID) Do begin
				...
				if PosCounter = 1 then
					if Not DB_Update(DB_ID, 'UPDATE Accounts SET Gold = Gold + 1, Points = Points + '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' WHERE Name = '''+EscapeApostrophe(DB_GetString(DB_ID, 0))+''';') then
						WriteLn('RunModeError16: '+DB_Error);

				if PosCounter = 2 then
					if Not DB_Update(DB_ID, 'UPDATE Accounts SET Silver = Silver + 1, Points = Points + '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' WHERE Name = '''+EscapeApostrophe(DB_GetString(DB_ID, 0))+''';') then
						WriteLn('RunModeError16: '+DB_Error);

				if PosCounter = 3 then
					if Not DB_Update(DB_ID, 'UPDATE Accounts SET Bronze = Bronze + 1, Points = Points + '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' WHERE Name = '''+EscapeApostrophe(DB_GetString(DB_ID, 0))+''';') then
						WriteLn('RunModeError16: '+DB_Error);
...
function MapBestScore(MapName: String): TDateTime;
begin
	if Not DB_Query(DB_ID, 'SELECT Score FROM Scores WHERE Map = '''+EscapeApostrophe(MapName)+''' ORDER BY Score DESC, HeadShots DESC;') then
...
function PlayerBestScore(Account, MapName: String): TDateTime;
begin
	if Not DB_Query(DB_ID, 'SELECT Score, Id FROM Scores WHERE Account = '''+EscapeApostrophe(Account)+''' AND Map = '''+EscapeApostrophe(MapName)+''' LIMIT 1;') then
...
var
	...
	TempScore, TempScore2: TDateTime;
	Score, Acc, Hits, Shots: Single;
begin
	...
	for i := 1 to 32 do
		if Players[i].Active then begin
			...
			// Needs tweaking
			Score :=   ((0.33 * _HeadShots[i] + 0.3 * (_ShotsHit[i]-_HeadShots[i])- 0.2 * (_ShotsFired[i]-_ShotsHit[i]))/(0.28 * (Now - _Timer[i])))/1000;

			if (Players[i].KeyShoot) then begin		
				if Ticks mod 7 = 0 then begin
					_ShotsFired[i] := _ShotsFired[i] + 1;
				end;					
			end;
			...
			if _ShowTimer[i] then begin
				if _Laps = 0 then begin
					Players[i].BigText(3, 'Shots fired: (' + IntToStr(_HeadShots[i]) + ') '+ IntToStr(_ShotsHit[i]) +'/'+IntToStr(_ShotsFired[i])+#10+FormatFloat('0.00',Score), 120, $FFFFFF, 0.1, 320, 360)
				end else begin
					Players[i].BigText(3, 'Shots fired: (' + IntToStr(_HeadShots[i]) + ') '+ IntToStr(_ShotsHit[i]) +'/'+IntToStr(_ShotsFired[i])+#10+FormatFloat('0.00',Score)+', Lap: '+IntToStr(_LapsPassed[i]+1)+'/'+IntToStr(_Laps), 120, $FFFFFF, 0.1, 320, 360)
				end;
			end;
			...
			if (_RealTimer[i] > 0) then begin
				Hits := _ShotsHit[i];
				Shots := _ShotsFired[i];
				Acc := Hits/Shots;
				Players.WriteConsole('STATS: Accuracy: '+FormatFloat('0.00',Acc*100)+'% [('+IntToStr(_HeadShots[i])+') '+IntToStr(_ShotsHit[i])+'/'+IntToStr(_ShotsFired[i])+'] Time: ' +ShowTime(_RealTimer[i]), $BFAFB2);
				if Not DB_Query(DB_ID, 'SELECT Name FROM Accounts WHERE Name = '''+EscapeApostrophe(Players[i].Name)+''' LIMIT 1;') then
					...
					if DB_NextRow(DB_ID) then begin //If account was found
						TempScore := MapBestScore(Game.CurrentMap);
						if TempScore = -1 then begin
							
							Players.WriteConsole('[1] First score by '+Players[i].Name+': '+FormatFloat('0.00',Score)+' points', $FFD700);
							
							if _LoggedIn[i] then begin
						
								DB_Update(DB_ID, 'BEGIN TRANSACTION;');
									
									if Not DB_Update(DB_ID, 'INSERT INTO Scores(Account, Map, Date, Time, HeadShots, Hits, Bullets, Accuracy, Score) VALUES('''+EscapeApostrophe(Players[i].Name)+''', '''+EscapeApostrophe(Game.CurrentMap)+''', '+FloatToStr(Now)+', '+FloatToStr(_RealTimer[i])+', '+IntToStr(_HeadShots[i])+', '+IntToStr(_ShotsHit[i])+', '+IntToStr(_ShotsFired[i])+', '+FloatToStr(Acc)+', '+FloatToStr(Score)+');') then //Add score
							...
							begin
								TempScore2 := PlayerBestScore(Players[i].Name, Game.CurrentMap);
								
								if TempScore2 = -1 then begin
									...
									DB_Update(DB_ID, 'BEGIN TRANSACTION;');
										...
										if Not DB_Query(DB_ID, 'SELECT Account FROM Scores WHERE Map = '''+EscapeApostrophe(Game.CurrentMap)+''' ORDER BY Score DESC, HeadShots DESC;') then //Find position
										...
										if PosCounter = 1 then
											Players.WriteConsole('[1] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best score: Not found, Map''s best score: +'+FormatFloat('0.00',Score-TempScore)+' points', $FFD700);
											
										if PosCounter = 2 then
											Players.WriteConsole('[2] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best score: Not found, Map''s best time: '+FormatFloat('0.00',Score-TempScore)+' points', $C0C0C0);
											
										if PosCounter = 3 then
											Players.WriteConsole('[3] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: Not found, Map''s best score: '+FormatFloat('0.00',Score-TempScore)+' points', $F4A460);
											
										if PosCounter > 3 then
											Players.WriteConsole('['+IntToStr(PosCounter)+'] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: Not found, Map''s best score: '+FormatFloat('0.00',Score-TempScore)+' points', COLOR_1);
									...
									if Score <= TempScore2 then
										Players.WriteConsole(+FormatFloat('0.00',Score)+' points was worse than '+Players[i].Name+'''s best by: '+FormatFloat('0.00',Score-TempScore2), COLOR_1)
									else begin
										...
										DB_Update(DB_ID, 'BEGIN TRANSACTION;');
											...
											if PosCounter = 1 then
												Players.WriteConsole('[1] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: +'+FormatFloat('0.00',Score-TempScore2)+', Map''s best time: +'+FormatFloat('0.00',Score-TempScore)+' points', $FFD700);
												
											if PosCounter = 2 then
												Players.WriteConsole('[2] '++FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: +'+FormatFloat('0.00',Score-TempScore2)+', Map''s best time: '+FormatFloat('0.00',Score-TempScore)+' points', $C0C0C0);
												
											if PosCounter = 3 then
												Players.WriteConsole('[3] '++FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: +'+FormatFloat('0.00',Score-TempScore2)+', Map''s best time: '+FormatFloat('0.00',Score-TempScore)+' points', $F4A460);
												
											if PosCounter > 3 then
												Players.WriteConsole('['+IntToStr(PosCounter)+'] '+FormatFloat('0.00',Score)+' points, '+Players[i].Name+'''s best time: +'+FormatFloat('0.00',Score-TempScore2)+', Map''s best time: '+FormatFloat('0.00',Score-TempScore)+' points', COLOR_1);
			...
			if _FlightMode[i] then begin
				...
				if Distance(Players[i].X, Players[i].Y, _LastPosX[i], _LastPosY[i]) < 30 then begin
					Players[i].Move(_LastPosX[i], _LastPosY[i])
				end;
				_LastPosX[i] := Players[i].X;
				_LastPosY[i] := Players[i].Y;
			end;

			if _GodMode[i] then begin
				Players[i].SetVelocity(0, 0);
			...
			if Ticks mod 3 = 0 then
				if (Players[i].KeyFlagThrow) and (_RKill[i]) and (not _EditMode) and (not _FlightMode[i]) and (not _GodMode[i]) then
					if length(_CheckPoint) <= 1 then
						Players[i].BigText(5, 'Checkpoints error', 120, $FF0000, 0.1, 320, 300)
					else begin
						Players[i].Damage(i, 150);
						//DAMAGE ASSOCIATED BOT
						{if _isPlaying[i] then begin
							Players[_AssociatedBot[i]].Damage(_AssociatedBot[i], 150);
			...
			if Ticks mod 60 = 0 then begin
				...
				if (not _LoggedIn[i]) and (Players[i].Team <> 5) then
					Players[i].BigText(4, 'Not logged in', 120, $FF0000, 0.1, 320, 20);
				
				if (Players[i].Alive) and (not _EditMode) and (not _FlightMode[i]) and (not _GodMode[i]) then
					if (Not _isPlaying[i]) then
						Players[i].BigText(6, 'Type !play'+#10+ 'Press "Flag Throw Key" to retry', 120, COLOR_1, 0.1, 320, 270);
...
Result := False;
	...
	if Command = '/damage' then
		if _ShowDamage[Player.ID] then
			_ShowDamage[Player.ID] := FALSE
		else
			_ShowDamage[Player.ID] := TRUE;
...
begin
	...
	if Text = '!help' then begin
		Player.WriteConsole('Welcome to Aim Lab!', COLOR_1);
		Player.WriteConsole('First of all, you have to make an account with command /account <password>,', COLOR_2);
		Player.WriteConsole('otherwise your scores won''t be recorded, then login witht the command /login <password>.', COLOR_2);
		Player.WriteConsole('After logging in type /aladd to login automatically in the future', COLOR_2);
		Player.WriteConsole('Please read !howto to learn how to play the game.', COLOR_2);
	end;
	
	if Text = '!howto' then begin
		Player.WriteConsole('How to play Aim Lab!', COLOR_1);
		Player.WriteConsole('This mode is about practicing your movement and your aim on bots that stand still and moves when you kill it', COLOR_2);
		Player.WriteConsole('To start playing type !play. A bot will appear with your same shirt color, shoot it a few times until it dies (under 80 HP) and moves.', COLOR_2);
		Player.WriteConsole('Go find the next one and do the same until you have finished the circuit.', COLOR_2);
		Player.WriteConsole('Read !info if you are interested in learning more.', COLOR_2);
	end;

	if Text = '!info' then begin
		Player.WriteConsole('Aim Lab Information!', COLOR_1);
		Player.WriteConsole('Aim Lab is currently under development and so is the way the scores are calculated. ', COLOR_2);
		Player.WriteConsole('Right now the scores are calculated based on (HeadShots + Hits - Misses)/Time', COLOR_2);
		Player.WriteConsole('For more information or suggestions ask mipHo on Discord.', COLOR_2);
	end;

	if Text = '!cmds' then begin
		Player.WriteConsole('Aim Lab commands:', COLOR_1);
		Player.WriteConsole('!play - To start playing', COLOR_2);
		...
		Player.WriteConsole('/damage - Enable/Disable showing damage', COLOR_2);
		Player.WriteConsole('More commands on !commands', COLOR_2);
	end;
	...
	if Text = '!top' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Game.CurrentMap)+''' ORDER BY Score DESC, HeadShots DESC LIMIT 3;') then
			...
			While DB_NextRow(DB_ID) Do begin
				...
				if PosCounter = 1 then
					Players.WriteConsole('[1] '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', $FFD700);
						
				if PosCounter = 2 then
					Players.WriteConsole('[2] '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', $C0C0C0);
					
				if PosCounter = 3 then
					Players.WriteConsole('[3] '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', $F4A460);
	...
	if (Copy(Text, 1, 5) = '!top ') and (Copy(Text, 6, Length(Text)) <> nil) then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Copy(Text, 6, Length(Text)))+''' ORDER BY Score DESC, HeadShots DESC LIMIT 3;') then
	...
	if Text = '!top10' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Game.CurrentMap)+''' ORDER BY Score DESC, HeadShots DESC LIMIT 10;') then
			...
			While DB_NextRow(DB_ID) Do begin
				...
				if PosCounter > 3 then
					Players.WriteConsole('['+IntToStr(PosCounter)+'] '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_1);
	...
	if (Copy(Text, 1, 7) = '!top10 ') and (Copy(Text, 8, Length(Text)) <> nil) then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Copy(Text, 8, Length(Text)))+''' ORDER BY Score DESC, HeadShots DESC LIMIT 10;') then
			WriteLn('RunModeError83: '+DB_Error)
		else begin
			While DB_NextRow(DB_ID) Do begin
				Inc(PosCounter, 1);
	...
	if Text = '!top20' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Game.CurrentMap)+''' ORDER BY Score DESC, HeadShots DESC LIMIT 20;') then
			WriteLn('RunModeError83: '+DB_Error)
		else begin
			While DB_NextRow(DB_ID) Do begin
				Inc(PosCounter, 1);
	...
	if (Copy(Text, 1, 7) = '!top20 ') and (Copy(Text, 8, Length(Text)) <> nil) then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time FROM Scores WHERE Map = '''+EscapeApostrophe(Copy(Text, 8, Length(Text)))+''' ORDER BY Score DESC, HeadShots DESC LIMIT 20;') then
			WriteLn('RunModeError83: '+DB_Error)
		else begin
			While DB_NextRow(DB_ID) Do begin
				Inc(PosCounter, 1);
	...
	if Text = '!last10scores' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores ORDER BY Date DESC LIMIT 10;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
		DB_FinishQuery(DB_ID);
	end;
	
	if Text = '!last20scores' then begin
		Player.WriteConsole('Last 20 scores', COLOR_1);
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores ORDER BY Date DESC LIMIT 20;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
	...
	{$IFDEF AIMLAB}
		if Text = '!elite' then begin
			Player.WriteConsole('Points: based on score', COLOR_1);
	...
	if Text = '!last10' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores WHERE Account = '''+EscapeApostrophe(Player.Name)+''' ORDER BY Date DESC LIMIT 10;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
	...
	if (Copy(Text, 1, 8) = '!last10 ') and (Copy(Text, 9, Length(Text)) <> nil) then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores WHERE Account = '''+EscapeApostrophe(Copy(Text, 9, Length(Text)))+''' ORDER BY Date DESC LIMIT 10;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
	...
	if Text = '!last20' then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores WHERE Account = '''+EscapeApostrophe(Player.Name)+''' ORDER BY Date DESC LIMIT 20;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
	...
	if (Copy(Text, 1, 8) = '!last20 ') and (Copy(Text, 9, Length(Text)) <> nil) then begin
		...
		if Not DB_Query(DB_ID, 'SELECT Account, Date, Score, Id, HeadShots, Hits, Bullets, Accuracy, Time, Map FROM Scores WHERE Account = '''+EscapeApostrophe(Copy(Text, 9, Length(Text)))+''' ORDER BY Date DESC LIMIT 20;') then
			WriteLn('RunModeError85: '+DB_Error)
		else
			While DB_NextRow(DB_ID) Do
				Players.WriteConsole('Map: '+DB_GetString(DB_ID, 9)+', Score: '+FormatFloat('0.00',DB_GetDouble(DB_ID, 2))+' points by '+DB_GetString(DB_ID, 0)+', Accuracy '+FormatFloat('0.00',DB_GetDouble(DB_ID, 7)*100)+'% [('+IntToStr(DB_GetLong(DB_ID, 4))+') '+IntToStr(DB_GetLong(DB_ID, 5))+'/'+IntToStr(DB_GetLong(DB_ID, 6))+'] Time: '+ShowTime(DB_GetDouble(DB_ID, 8))+'s on '+FormatDateTime('yyyy.mm.dd hh:nn:ss', DB_GetDouble(DB_ID, 1))+' ['+DB_GetString(DB_ID, 3)+']', COLOR_2);
	...
	if Text = '!play' then begin
		if (Player.Name <> 'Dummy') and (Not _isPlaying[Player.ID]) then begin
			NewPlayer := TNewPlayer.Create;
			try
				NewPlayer.Name := 'Dummy';
				NewPlayer.Team := 2;
				NewPlayer.Dummy := True;
				NewPlayer.ShirtColor := Player.ShirtColor;
				NewPlayer.FavouriteWeapon := 'Hands';
				NewDummy := Players.Add(NewPlayer, TJoinSilent);
				_AssociatedBot[Player.ID] := NewDummy.ID;
				_AssociatedBot[NewDummy.ID] := Player.ID;
				_LastPosX[_AssociatedBot[Player.ID]] := _CheckPoint[0].X;
				_LastPosY[_AssociatedBot[Player.ID]] := _CheckPoint[0].Y;				
				_ShowTimer[Player.ID] := True;
			finally
				NewPlayer.Free;
			end;
			_isPlaying[Player.ID] := True;
			Player.ChangeTeam(1, TJoinNormal);
			Players[Player.ID].Damage(Player.ID, 150);
			{$IFDEF DEBUG_AIMLAB}
				Player.WriteConsole(Player.Name + ': ' + IntToStr(_AssociatedBot[Player.ID]), COLOR_1);
			{$ENDIF}
		end;
	end;
end;
...
begin
	...
	if (_AssociatedBot[Player.ID] <> 0) then begin
		Player.WorldText(1, '', 9999, $00FFFF, 0.05, _CheckPoint[_CheckPointPassed[Player.ID]].X+10*-0.3, _CheckPoint[_CheckPointPassed[Player.ID]].Y+110*-0.3);
		if (Not Players[_AssociatedBot[Player.ID]].Human) then begin
			Players[_AssociatedBot[Player.ID]].Kick(TKickSilent);
		end;
		_AssociatedBot[Player.ID] := 0;
		_isPlaying[Player.ID] := False;
	end;
...
begin
	...
	_GodMode[Player.ID] := False;
	_FlightMode[Player.ID] := False;
	if (Player.Human) and (_isPlaying[Player.ID]) then begin
		if (Not Players[_AssociatedBot[Player.ID]].Human)  then
			Players[_AssociatedBot[Player.ID]].Kick(TKickSilent);
	end;
	_AssociatedBot[Player.ID] := 0;
	_isPlaying[Player.ID] := FALSE;
...
function LowerIdOnCheckPoint(ID: Integer): Integer;
var
	i: Integer;
begin
	Result := 33;
	for i := 1 to 32 do begin
		if ((Players[i].Human) and (_CheckPointPassed[ID] = _CheckPointPassed[i]) and (_AssociatedBot[i] <> 0)) then begin // both players have dummys on the same CP
			if (_AssociatedBot[i] < Result) then begin
				Result := _AssociatedBot[i];
			end;
		end;
	end;	
end;
...
var 
	...
	LowID: Integer;
begin
	// Fix for multiple bots on the same position making bullets lose speed and lowering its damage
	LowID := LowerIdOnCheckPoint(Shooter.ID);
	if Victim.ID <> 0 then begin
		_LastDamage[Victim.ID] := Damage;
	end;
	
	if (Not _GodMode[Victim.ID]) then begin
		// Count the total damage, shots and headshots and show info on screen
		if (Victim.ID = _AssociatedBot[Shooter.ID]) and (Not Victim.Human) then begin // Victim is bot
			_Damage[Shooter.ID] := _Damage[Shooter.ID] + Damage;
			_ShotsHit[Shooter.ID] := _ShotsHit[Shooter.ID] + 1;
			HS := 0.678 * sqrt(Map.Bullets[BulletId].VelX * Map.Bullets[BulletId].VelX + Map.Bullets[BulletId].VelY * Map.Bullets[BulletId].VelY)

			// Count headshots
			if (_ShotsFired[Shooter.ID] < _ShotsHit[Shooter.ID]) then
				_ShotsFired[Shooter.ID] := _ShotsHit[Shooter.ID];			
			if (Damage > HS) then begin
				_HeadShots[Shooter.ID] := _HeadShots[Shooter.ID] + 1;
			end;
			
			// Showing damage on screen
			if (_ShowDamage[Shooter.ID]) then begin
				if (Damage > HS) then begin
					Shooter.WorldText(4, FormatFloat('0.00',_LastDamage[LowID]), 80, $FF0000, 0.035, _CheckPoint[_CheckPointPassed[Shooter.ID]].X+90*-0.3, _CheckPoint[_CheckPointPassed[Shooter.ID]].Y+100*-0.3);
				end else begin
					Shooter.WorldText(4, FormatFloat('0.00',_LastDamage[LowID]), 80, $FFFF00, 0.035, _CheckPoint[_CheckPointPassed[Shooter.ID]].X+90*-0.3, _CheckPoint[_CheckPointPassed[Shooter.ID]].Y+100*-0.3);
				end;
			end;
			
			if Victim.Health-Damage < 70 then begin
				Shooter.WorldText(2, '0/90', 80, $FF0000, 0.05, _CheckPoint[_CheckPointPassed[Shooter.ID]].X+80*-0.3, _CheckPoint[_CheckPointPassed[Shooter.ID]].Y+130*-0.3);
			end else 
				Shooter.WorldText(2, FormatFloat('0',Victim.Health-60-Damage) + '/90', 80, $AAFF00, 0.05, _CheckPoint[_CheckPointPassed[Shooter.ID]].X+80*-0.3, _CheckPoint[_CheckPointPassed[Shooter.ID]].Y+130*-0.3);
		end;

		// Damage logic
		if ((Not Victim.Human) and (Victim.ID = _AssociatedBot[Shooter.ID])) and ((Players[Shooter.ID].Active) and (Victim.Health-Damage < 70))  and (Not _JustFinished[Shooter.ID]) then begin
			Result := 0;			
			
			if _CheckPointPassed[Shooter.ID] < High(_CheckPoint) then begin
				_CheckPointPassed[Shooter.ID] := _CheckPointPassed[Shooter.ID] + 1;
				Victim.Move(_CheckPoint[_CheckPointPassed[Shooter.ID]].X,_CheckPoint[_CheckPointPassed[Shooter.ID]].Y);
				Shooter.WorldText(1, 'v', 9999, $00FFFF, 0.05, _CheckPoint[_CheckPointPassed[Shooter.ID]].X+10*-0.3, _CheckPoint[_CheckPointPassed[Shooter.ID]].Y+110*-0.3);
				Victim.SetVelocity(0,0);				
				Victim.Health := 150;
			end else begin
				_RealTimer[Shooter.ID] := Now - _Timer[Shooter.ID];
				_JustFinished[Shooter.ID] := True;
				Shooter.Damage(Shooter.ID, 150);
			end;		
		end else if  (Victim.ID = Shooter.ID) then begin
			Result := Damage;
		end else if (Victim.ID = _AssociatedBot[Shooter.ID]) then begin
			Result := _LastDamage[LowID];
		end else
			Result := 0;
	end else begin
		Result := 0;
...
begin
	_Damage[Player.ID] := 0;
	_ShotsFired[Player.ID] := 0;
	_ShotsHit[Player.ID] := 0;
	_HeadShots[Player.ID] := 0;
	if length(_CheckPoint) > 1 then begin
...
begin
	if length(_CheckPoint) > 1 then begin
		if (_JustDied[Player.ID]) and (Player.Human) and _isPlaying[Player.ID] then begin
			...
			// Move the bot to the first cp
			_FlightMode[_AssociatedBot[Player.ID]] := True;
			Players[_AssociatedBot[Player.ID]].Move(_CheckPoint[_CheckPointPassed[Player.ID]].X,_CheckPoint[_CheckPointPassed[Player.ID]].Y);
			Player.WorldText(1, 'v', 500, $00FFFF, 0.05, _CheckPoint[_CheckPointPassed[Player.ID]].X+10*-0.3, _CheckPoint[_CheckPointPassed[Player.ID]].Y+110*-0.3);
			Players[_AssociatedBot[Player.ID]].SetVelocity(0,0);
			if (Players[_AssociatedBot[Player.ID]].Health<150) then 
				Players[_AssociatedBot[Player.ID]].Health := 150;
			{$IFDEF RUN_MODE_DEBUG}
				_PingRespawn[Player.ID] := Player.Ping;
			{$ENDIF}
			_JustDied[Player.ID] := False;
			_JustFinished[Player.ID] := False;
		end else
			begin
				SetLength(_Record[Player.ID], 0);
				_LapsPassed[Player.ID] := 0;
...
begin
	if not File.Exists(DB_NAME) then begin
		...
		if DatabaseOpen(DB_ID, DB_NAME, '', '', DB_Plugin_SQLite) then begin
			...
			Query := Query+'Points DOUBLE DEFAULT 0,';
			Query := Query+'Email TEXT,';
			Query := Query+'Bookmarks TEXT,';
			Query := Query+'PremiumExpiry DOUBLE);';
			DatabaseUpdate(DB_ID, Query);
			DatabaseUpdate(DB_ID, 'CREATE TABLE Scores(Id INTEGER PRIMARY KEY, Account TEXT, Map TEXT, Date DOUBLE, Time DOUBLE, HeadShots INTEGER, Hits INTEGER, Bullets INTEGER, Accuracy DOUBLE, Score DOUBLE);');
		end;
	end else
